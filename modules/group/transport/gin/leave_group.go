...
func LeaveGroup(appCtx appcontext.AppContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := leaveGroupBiz.Leave(c.Request.Context(), requesterId, groupId); err != nil {
			panic(err)
		}
		...
		go func() {
			...
			chatItem := &gchatmdl.GroupChatItem{
				...
				GroupId:     groupId,
				SystemEvent: common.GetPointer(gchatmdl.MemberLeaved),
			}
			_ = chatItem.Process()
			err := biz.Send(context.Background(), chatItem)
			if err != nil {
				log.Error().Err(err).Str("package", "groupgin.LeaveGroup.Send").Send()
			}
		}()
